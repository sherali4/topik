...
from func import replace_text_in_doc, eng_katta, kiril_to_latin
import openpyxl
...
def from_excel(kitob, yacheyka):
    ...
    value = sheet[yacheyka].value  # type: ignore
    workbook.close()
...
def replace_text_in_doc(doc_path, file_path, old_text, new_text):
    try:
        ...
        # Paragraflardagi matnni almashtirish
        for para in doc.paragraphs:
            for run in para.runs:
                if old_text in run.text:
                    run.text = run.text.replace(old_text, new_text)

        # Jadvallardagi matnni almashtirish
        ...
        # Sarlavha va pastki sarlavhalarda matnni almashtirish
        ...
        try:
            doc.save(file_path)
            print(f"'{file_path}' fayli saqlandi. '{old_text}' matni '{new_text}' matniga o'zgartirildi.")
        except PermissionError:
            print(f"Faylni saqlashda ruxsat yo'q: {file_path}")
