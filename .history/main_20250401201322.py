...
def get_excel_data_as_dict(file_path, column, start_row, end_row):
    ...
    for row in range(start_row, end_row + 1):
        ...
        data_dict[f"{column}{row}"] = str(cell_value).replace(
            '.', ',') if cell_value else ''
...
# Foydalanish
excel_data = get_excel_data_as_dict("baza.xlsx", "B", 4, 18)
print(excel_data)
