...
def get_sorted_excel_data_desc(file_path, column, start_row, end_row):
    ...
    for row in range(start_row, end_row + 1):
        ...
        if cell_value is not None:
            try:
                ...
                data_dict[f"{column}{row}"] = numeric_value
            except ValueError:
                # Agar noto‘g‘ri qiymat bo‘lsa, 0 qo‘yamiz
                data_dict[f"{column}{row}"] = 0

    workbook.close()

    # Qiymatlar bo‘yicha kamayish tartibida saralash (Z → A)
    sorted_data = dict(
...
# Foydalanish
sorted_excel_data_desc = get_sorted_excel_data_desc("baza.xlsx", "B", 4, 18)
